...
MAX_RECEIVED_BYTES: Final = 10240
...
HTTP_VER: Final = 'HTTP/1.1 '
HTTP_VERB_GET: Final = 'GET'
HTTP_VERB_POST: Final = 'POST'

HTTP_HEADER_200: Final = '200 OK'
HTTP_HEADER_400: Final = '400 Bad Request'
HTTP_HEADER_401: Final = '401 Unauthorized'
HTTP_HEADER_404: Final = '404 Not Found'
HTTP_HEADER_405: Final = '405 Method Not Allowed'
HTTP_HEADER_500: Final = '500 Internal Server Error'
HTTP_HEADER_503: Final = '503 Service Unavailable'

HTTP_HEADER_CONTENT_TYPE: Final = 'Content-Type: '
HTTP_HEADER_WWW_AUTHENTICATE: Final = 'WWW-Authenticate: Basic realm=\'Portfolio\''
...
def loadAndReturnFile(requestUri):
    try:
        ...
        http200ok: Final = HTTP_VER + HTTP_HEADER_200 + LINE_ENDING

        contentType = HTTP_HEADER_CONTENT_TYPE
        if fullPath.endswith(".ico"):
            contentType += 'image/vnd.microsoft.icon' + LINE_ENDING
        elif fullPath.endswith(".htm") or fullPath.endswith(".html"):
            contentType += 'text/html' + LINE_ENDING
        elif fullPath.endswith(".css"):
            contentType += 'text/css' + LINE_ENDING
        elif fullPath.endswith(".js"):
            contentType += 'text/javascript' + LINE_ENDING
        else:
            contentType = ''

        return http200ok + contentType, getFileContents(fullPath)

    except IOError as e:
        log('loadAndReturnFile: caught [' + str(e) + ']')
...
def getPasswordFromFile():
    return getFileContents('resources/definitelyNotAPasswordFile.txt').decode('UTF-8').strip()
...
def getAllStockSymbols():
    ...
    if stockSymbols == IEX_PLEASE_UPGRADE_MESSAGE:
        log('using the cached stock symbols')
        return json.loads(getFileContents('exampleData/stock_symbols.js'))
...
def getStockQuote(stockSymbol):
    ...
    if stockQuote == IEX_PLEASE_UPGRADE_MESSAGE:
        log('using the cached stock quote')
        return json.loads(getFileContents('exampleData/stock_quote.js'))
...
def extractFormVariables(rawFormVariables):
    if rawFormVariables is None:
        log('rawFormVariables is None')
        return {}

    varNameAndValue = rawFormVariables.split('&')
    if varNameAndValue is None or len(varNameAndValue) == 0:
        log('varNameAndValue is None or len(varNameAndValue) == 0')
        return {}

    form = {}
    log('varNameAndValue = ' + str(varNameAndValue))
...
def getStockStatistics(stockSymbol):
    ...
    if stockStatistics == IEX_PLEASE_UPGRADE_MESSAGE:
        log('using the cached stock statistics')
        stockStatistics = getFileContents('exampleData/stock_statistics.js')
...
def getChartDataPoints(stockSymbol):
    ...
    if dataPoints == IEX_PLEASE_UPGRADE_MESSAGE:
        log('using the cached data points')
        return getFileContents('exampleData/stock_data_points.js')
...
def makeResponse(httpCode, parameter):
    match httpCode:
        case 200:
            header = HTTP_HEADER_200
            body = None
        case 400:
            header = HTTP_HEADER_400
            body = getFileContents('errorPages/400.html')
        case 401:
            header = HTTP_HEADER_401 + LINE_ENDING + HTTP_HEADER_WWW_AUTHENTICATE
            body = getFileContents('errorPages/401.html')
        case 404:
            header = HTTP_HEADER_404
            body = getFileContents('errorPages/404.html')
        case 405:
            header = HTTP_HEADER_405
            body = getFileContents('errorPages/405.html')
        case 503:
            header = HTTP_HEADER_503
            body = getFileContents('errorPages/503.html')
        case _:
            header = HTTP_HEADER_500
            body = generateHttp500Page(parameter).encode()
    return HTTP_VER + header + LINE_ENDING, body
...
def processHttpRequest(password, requestHeaders, requestBody):
    ...
    credentials = base64.b64decode(authorization[2]).decode('UTF-8').split(':')
    ...
    try:
        ...
        if requestLine[0] == HTTP_VERB_GET:
            ...
            if requestLine[1] == RESEARCH_URI:
                ...
                responseHeaders += HTTP_HEADER_CONTENT_TYPE + 'text/html' + LINE_ENDING
            ...
            # log(str(requestHeaders))
            form = extractFormVariables(requestBody)
...
def processTcpConnection(remoteSocket):
    ...
    try:
        ...
        requestBytes = remoteSocket.recv(MAX_RECEIVED_BYTES)
        if len(requestBytes) == 0:
            closeReason = 'No bytes read from client socket'
        else:
            requestHeaders = requestBytes.decode('UTF-8').splitlines(False)  # convert the text to an array of text lines
            if requestHeaders is None or len(requestHeaders) < 1:
                responseHeaders, responseBody = makeResponse(400, None)
            else:
                requestedResource = requestHeaders[0]
                requestBody = ''
                contentLength = getHeader('Content-Length', requestHeaders)
                if contentLength is not None:
                    requestBody = remoteSocket.recv(int(contentLength[1])).decode('UTF-8')
                responseHeaders, responseBody = processHttpRequest(passwordFromFile, requestHeaders, requestBody)

            log(responseHeaders.split(LINE_ENDING)[0])
            remoteSocket.sendall((responseHeaders + LINE_ENDING).encode())

            if responseBody is not None:
                remoteSocket.sendall(responseBody)

            closeReason = 'Request completed'

    except Exception as e:
        closeReason = 'Exception: ' + str(e)
        log('Caught [' + str(e) + ']')
    finally:
        log('Closing connection, reason [' + closeReason + '], request [' + requestedResource + ']')
        remoteSocket.close()
...
http500Page = pybars.compile(getFileContents('errorPages/500.html').decode('UTF-8'))
...
serviceUnavailable = (HTTP_VER + HTTP_HEADER_503 + LINE_ENDING + LINE_ENDING).encode()

log('The server is ready to receive HTTP requests.')
...
while True:
    try:
        ...
        if passwordFromFile is None:
            ...
            clientSocket.sendall(serviceUnavailable)
            clientSocket.sendall(getFileContents('errorPages/503.html'))
            clientSocket.close()
        else:
